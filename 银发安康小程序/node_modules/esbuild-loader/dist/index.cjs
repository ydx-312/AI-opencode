...
async function ESBuildLoader(source) {
  ...
  if (!("tsconfigRaw" in transformOptions) && (!isDependency || tsExtensionsPattern.test(resourcePath))) {
    if (!isDependency && tsconfigPath) {
      ...
      if (!tsconfig) {
        tsconfig = {
          config: getTsconfig.parseTsconfig(tsconfigFullPath),
      ...
      let tsconfig;
      try {
        tsconfig = getTsconfig.getTsconfig(resourcePath, "tsconfig.json");
      } catch (error) {
...
class EsbuildPlugin {
  ...
  apply(compiler) {
    ...
    const isEvalDevtool = typeof devtool === "string" && devtool.includes("eval");
    let hasWarnedEvalDefine = false;
    compiler.hooks.compilation.tap(pluginName, (compilation) => {
      if (!hasWarnedEvalDefine && options.define && Object.keys(options.define).length > 0 && isEvalDevtool) {
        hasWarnedEvalDefine = true;
        const warning = new Error(
          `[${pluginName}] The "define" option may not work as expected with eval-based devtools (current: "${devtool}"). Eval devtools wrap module code in eval() strings, preventing identifier replacement. Consider using the loader's "define" option instead, or switch to a non-eval devtool like "source-map".`
        );
